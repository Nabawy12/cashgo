...
// search products by name or barcode (returns ALL matches)
  ...
  /// رجّع ملخّص مبيعات لكل يوزر دوره 'cashier'
  /// fromDate / toDate بصيغة 'YYYY-MM-DD' (اختياريان). لو لم يعطَوا، تحسب عن كل الزمن.
  /// excludeDrawerWithdrawn: لو true سيتم استثناء الفواتير الموسومة drawer_withdrawn = 1 إن وجد العمود.
  Future<List<Map<String, dynamic>>> getNetSalesByCashiers({
    String? fromDate,
    String? toDate,
    bool excludeDrawerWithdrawn = true,
  }) async {
    final db = await instance.database;

    // 1) جلب أسماء الكاشيرز
    final users = await db.query('users', where: 'role = ?', whereArgs: ['cashier']);
    final cashierUsernames = users.map((u) => (u['username'] as String?) ?? '').where((s) => s.isNotEmpty).toList();

    // 2) تحقق من أعمدة drawer (للاحتساب الصحيح)
    final cols = await db.rawQuery("PRAGMA table_info(sales);");
    final hasDrawerWithdrawn = cols.any((c) => (c['name'] as String) == 'drawer_withdrawn');
    final hasDrawerWithdrawnAmount = cols.any((c) => (c['name'] as String) == 'drawer_withdrawn_amount');

    // 3) بناء شرط التاريخ ووسيطات الاستعلام
    String dateCond = '';
    final List<Object?> dateArgs = [];
    if (fromDate != null && toDate != null) {
      dateCond = ' AND date(date) BETWEEN ? AND ?';
      dateArgs.addAll([fromDate, toDate]);
    } else if (fromDate != null) {
    ...
    // 4) استعلام مجمّع على جدول sales (حسب كل كاشير)
    String salesCashExpr;
    if (hasDrawerWithdrawnAmount) {
      salesCashExpr =
      "SUM(CASE WHEN payment_method = 'cash' THEN CASE WHEN ((COALESCE(paid_amount,0)-COALESCE(change_amount,0)) - COALESCE(drawer_withdrawn_amount,0)) > 0 THEN ((COALESCE(paid_amount,0)-COALESCE(change_amount,0)) - COALESCE(drawer_withdrawn_amount,0)) ELSE 0 END ELSE 0 END) as sales_net_cash";
    } else {
      salesCashExpr =
      "SUM(CASE WHEN payment_method = 'cash' THEN (COALESCE(paid_amount,0) - COALESCE(change_amount,0)) ELSE 0 END) as sales_net_cash";
    }

    String salesWhereExtra = '';
    final salesArgs = <Object?>[];
    if (hasDrawerWithdrawn && excludeDrawerWithdrawn) {
      salesWhereExtra = ' AND COALESCE(drawer_withdrawn,0) = 0';
    }
    salesArgs.addAll(dateArgs);

    final salesSql = '''
      SELECT cashier_username,
             SUM(COALESCE(total,0)) as sales_total,
             $salesCashExpr,
             SUM(CASE WHEN payment_method = 'card' OR lower(payment_method) LIKE '%card%' OR lower(payment_method) = 'wallet' THEN (COALESCE(paid_amount,0) - COALESCE(change_amount,0)) ELSE 0 END) as sales_net_card
      FROM sales
      WHERE 1=1
      $salesWhereExtra
      $dateCond
    ...
    final salesRows = await db.rawQuery(salesSql, salesArgs);

    // 5) تجميع المرتجعات حسب الكاشير (sale_returns.paid_delta)
    final returnsSql = '''
      SELECT cashier_username, SUM(COALESCE(paid_delta,0)) as returns_delta
      FROM sale_returns
      WHERE 1=1
      $dateCond
      GROUP BY cashier_username
    ''';
    final returnsRows = await db.rawQuery(returnsSql, dateArgs);

    // 6) مدفوعات الشراء التي سجّلها المستخدم (purchase_receipts.received_by)
    final purchasesSql = '''
      SELECT received_by as cashier_username, SUM(COALESCE(paid_amount,0)) as purchases_paid_cash
      FROM purchase_receipts
      WHERE 1=1
      ${fromDate != null || toDate != null ? ' AND date(created_at) ' : ''}
      ${fromDate != null && toDate != null ? 'BETWEEN ? AND ?' : fromDate != null ? '>= ?' : toDate != null ? '<= ?' : ''}
    ''';
    // purchaseArgs يعكس نفس dateArgs لكن على created_at
    final purchaseArgs = <Object?>[];
    if (fromDate != null && toDate != null) {
      purchaseArgs.addAll([fromDate, toDate]);
    } else if (fromDate != null) {
      purchaseArgs.add(fromDate);
    } else if (toDate != null) {
      purchaseArgs.add(toDate);
    }
    final purchasesRows = await db.rawQuery(purchasesSql, purchaseArgs);

    // 7) بناء خريطة سريعة من النتائج لتسهيل الدمج
    final Map<String, Map<String, dynamic>> salesMap = {};
    for (final r in salesRows) {
      final k = (r['cashier_username'] as String?) ?? '';
      salesMap[k] = {
        'sales_total': (r['sales_total'] as num?)?.toDouble() ?? 0.0,
        'sales_net_cash': (r['sales_net_cash'] as num?)?.toDouble() ?? 0.0,
        'sales_net_card': (r['sales_net_card'] as num?)?.toDouble() ?? 0.0,
      };
    }

    final Map<String, double> returnsMap = {};
    for (final r in returnsRows) {
      final k = (r['cashier_username'] as String?) ?? '';
      returnsMap[k] = (r['returns_delta'] as num?)?.toDouble() ?? 0.0;
    }

    final Map<String, double> purchasesMap = {};
    for (final r in purchasesRows) {
      final k = (r['cashier_username'] as String?) ?? '';
      purchasesMap[k] = (r['purchases_paid_cash'] as num?)?.toDouble() ?? 0.0;
    }

    // 8) جَمّع النتيجة النهائية لكل كاشير معروف في users
    final List<Map<String, dynamic>> out = [];
    for (final username in cashierUsernames) {
      final s = salesMap[username] ?? {'sales_total': 0.0, 'sales_net_cash': 0.0, 'sales_net_card': 0.0};
      final returnsDelta = returnsMap[username] ?? 0.0;
      final purchasesPaid = purchasesMap[username] ?? 0.0;

      final salesTotal = (s['sales_total'] as double?) ?? 0.0;
      final salesNetCash = (s['sales_net_cash'] as double?) ?? 0.0;
      final salesNetCard = (s['sales_net_card'] as double?) ?? 0.0;

      // التعريف المستخدم هنا:
      // net_sales_total = مجموع عمود total للفاتورة (sales_total) — هذا يعكس المجموع بعد تعديل الفاتورة إن طُبّقت مرتجعات على نفس الفاتورة.
      // net_cash_in_drawer_for_cashier = ما دخل فعلياً للكاشير نقداً = sales_net_cash + returns_delta - purchases_paid_cash
      final netCashInDrawerForCashier = salesNetCash + returnsDelta - purchasesPaid;
      out.add({
        'username': username,
        'sales_total': salesTotal,
        'sales_net_cash': salesNetCash,
        'sales_net_card': salesNetCard,
        'returns_delta': returnsDelta,
        'purchases_paid_cash': purchasesPaid,
        'net_cash_in_drawer': netCashInDrawerForCashier,
      });
    }

    // 9) رتب حسب sales_total تنازليًا (اختياري)
    out.sort((a, b) => (b['sales_total'] as double).compareTo(a['sales_total'] as double));
    return out;
  }
