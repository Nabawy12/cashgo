...
class _receiptsScreenState extends State<receiptsScreen> {
  ...
  Future<void> _loadSales({DateTime? date}) async {
    ...
    try {
      ...
      if (resp.statusCode != 200) throw Exception('Server returned ${resp.statusCode}: ${resp.body}');
      ...
      // We'll split remote invoices into top-level (original sales) and child (returns/exchanges)
      ...
      for (final e in remote) {
        ...
        // parse basic fields
        final int? id = (raw['id'] is num) ? (raw['id'] as num).toInt() : (int.tryParse(raw['id']?.toString() ?? '') ?? null);
        if (id == null) continue;

        final int? parentInvoiceId = (raw['parent_invoice_id'] is num)
            ? (raw['parent_invoice_id'] as num).toInt()
            : (raw['parent_invoice_id'] != null ? int.tryParse(raw['parent_invoice_id'].toString()) : null);

        // normalize meta if present (server's get_all_invoices already does this in PHP)
        final dynamic meta = raw['meta'] ?? raw['meta_json'];

        final mapEntry = {
          'id': id,
          'invoice_id': raw['invoice_id']?.toString() ?? '',
          'product_list': raw['product_list'],
          'total': (raw['total'] is num) ? (raw['total'] as num).toDouble() : (double.tryParse(raw['total']?.toString() ?? '') ?? 0.0),
          'paid_amount': (raw['paid_amount'] is num) ? (raw['paid_amount'] as num).toDouble() : (double.tryParse(raw['paid_amount']?.toString() ?? '') ?? 0.0),
          'change_amount': (raw['change_amount'] is num) ? (raw['change_amount'] as num).toDouble() : null,
          'payment_type': raw['payment_type']?.toString() ?? raw['paymentMethod']?.toString() ?? '',
          'cashier_username': raw['cashier_username']?.toString() ?? raw['cashierName']?.toString() ?? '',
          'date': raw['created_at'] ?? raw['date'] ?? raw['created'] ?? '',
          'updated_at': raw['updated_at'] ?? null,
          'is_canceled': raw['is_canceled'] ?? 0,
          'status': raw['status'] ?? '',
          'type': raw['type'] ?? 'sale',
          'parent_invoice_id': parentInvoiceId,
          'meta': meta,
          // pass-through possible server-side is_credit flag if present
          'is_credit_flag': raw['is_credit'] ?? raw['is_credit_flag'] ?? (meta is Map && meta['is_credit'] != null ? meta['is_credit'] : null),
        };

        if (parentInvoiceId == null) {
          topLevel.add(mapEntry);
        } else {
          // child invoice (return / exchange)
          childInvoices.add(mapEntry);
        }
      }

      // ----- HERE: exclude credit invoices -----
      List<Map<String, dynamic>> notCreditTopLevel = topLevel.where((inv) {
        // 1) check explicit is_credit flag if present
        final dynamic isCreditRaw = inv['is_credit_flag'];
        if (isCreditRaw != null) {
          if (isCreditRaw is num) {
            if (isCreditRaw.toInt() == 1) return false; // exclude
          } else if (isCreditRaw is bool) {
            if (isCreditRaw) return false; // exclude
          } else if (isCreditRaw is String) {
            final s = isCreditRaw.trim().toLowerCase();
            if (s == '1' || s == 'true' || s == 'yes') return false; // exclude
          }
        }
      ...
      List<Map<String, dynamic>> filtered = notCreditTopLevel;
      if (date != null) {
        filtered = notCreditTopLevel.where((s) {
      ...
      // Build return caches from childInvoices: group items and meta per parent invoice
      for (final child in childInvoices) {
        final parentId = (child['parent_invoice_id'] is num) ? (child['parent_invoice_id'] as num).toInt() : (int.tryParse(child['parent_invoice_id']?.toString() ?? '') ?? 0);
        if (parentId == 0) continue;

        // parse product_list (could be array or {meta:..., items: [...]})
        ...
        // convert items to return items format: returned items have negative qty (server may store negative for returns)
        for (final it in items) {
          final int pid = (it['product_id'] is num) ? (it['product_id'] as num).toInt() : 0;
          int qty = (it['qty'] is num) ? (it['qty'] as num).toInt() : 0;
          final price = (it['price'] is num) ? (it['price'] as num).toDouble() : 0.0;
          final name = (it['product_name'] ?? '').toString();

          // interpret sign: negative => returned (is_replacement=0), positive => replacement (is_replacement=1)
          final isReplacement = qty > 0 ? 1 : 0;
          final qtyAbs = qty.abs();

          final row = {
            'return_id': (child['id'] as int),
            'product_id': pid,
            'product_name': name,
            'price': price,
            'qty': qtyAbs,
          ...
          saleReturnItemsCache[parentId]!.add(row);
        }

        // compute paid_delta = paid_amount - refund_amount_in_meta (if meta present)
        ...
        if (child['meta'] is Map) {
          try {
            final metaMap = Map<String, dynamic>.from(child['meta']);
            if (metaMap.containsKey('refund_amount')) {
              refundAmountInMeta = (metaMap['refund_amount'] is num) ? (metaMap['refund_amount'] as num).toDouble() : (double.tryParse(metaMap['refund_amount']?.toString() ?? '') ?? 0.0);
            }
          } catch (_) {}
        }
        final double paid_delta = paidAmount - refundAmountInMeta;

        // save return meta record
        saleReturnsCache.putIfAbsent((child['parent_invoice_id'] ?? 0) as int, () => []);
        saleReturnsCache[(child['parent_invoice_id'] as int)]!.add({
          'id': child['id'],
          'date': child['date'] ?? child['created_at'] ?? DateTime.now().toIso8601String(),
          'paid_delta': paid_delta,
          'meta': child['meta'] ?? {},
          'type': child['type'] ?? 'return',
          'cashier': child['cashier_username'] ?? '',
        });
      }
  ...
  bool _matchesPaymentFilter(Map<String, dynamic> s) {
    ...
    final pmRaw = (s['payment_type'] ?? s['paymentMethod'] ?? s['return_note'] ?? '').toString().toLowerCase();
  ...
  Map<String, dynamic> _extractDiscountInfo(
    ...
    if (s.containsKey('original_total') && s['original_total'] != null) {
      ...
      originalTotal = (ot is num) ? ot.toDouble() : (double.tryParse(ot.toString()) ?? currentItemsTotal);
    ...
    if (disc is Map) {
      if (disc.containsKey('type')) discountTypeRaw = (disc['type'] ?? '').toString();
      if (disc.containsKey('value')) {
        final v = disc['value'];
        discountValueRaw = (v is num) ? v.toDouble() : (double.tryParse(v.toString()) ?? 0.0);
      }
    } else {
      final t = s['discount_type'] ?? s['discountType'] ?? (meta is Map ? meta['discount_type'] ?? meta['discountType'] : null);
      final v = s['discount_value'] ?? s['discountValue'] ?? (meta is Map ? meta['discount_value'] ?? meta['discountValue'] : null);
      if (t != null) discountTypeRaw = t.toString();
      if (v != null) discountValueRaw = (v is num) ? v.toDouble() : (double.tryParse(v.toString()) ?? 0.0);
    }

    final lcType = discountTypeRaw.toString().toLowerCase();
    final isPercent = lcType.contains('percent') || lcType.contains('%') || lcType.contains('perc');
    ...
    final effectiveTotal = (originalTotal - discountAmount).clamp(0.0, double.infinity);
  ...
  double _effectiveTotalForSaleHeader(Map<String, dynamic> s) {
    final saleId = (s['id'] is num) ? (s['id'] as num).toInt() : (int.tryParse(s['id']?.toString() ?? '') ?? 0);
    double currentItemsTotal = 0.0;
                ...
                children: [
                  ...
                  Theme(
                    ...
                    child: ChoiceChip(
                      ...
                      selectedColor: AppColorsDark.bgColor,
                      labelStyle: TextStyle(
                        color: _activeFilter == 'all' ? Colors.white : Colors.white70,
                        fontWeight: FontWeight.w600,
                      ),
                                    ...
                                    content: Column(
                                      children: [
                                        ...cashierSales.map((s) {
                                          ...
                                          final salePaidFromAPI = (s['paid_amount'] as num?)?.toDouble() ?? 0.0;
                                          final cashierFromSale = (s['cashier_username'] ?? '').toString();
                                          final cashier = cashierFromSale.isNotEmpty
                                              ? cashierFromSale
                                              : (_currentUser != null ? (_currentUser!['username'] ?? 'غير معروف') : 'غير معروف');
                                            ...
                                            child: AccordionWidget(
                                              ...
                                              content: Padding(
                                                ...
                                                child: Builder(builder: (context) {
                                                  ...
                                                  final info = _extractDiscountInfo(s, currentItemsTotal: currentItemsTotal);
                                                  final double originalTotalFromMeta = (info['original_total'] as double?) ?? currentItemsTotal;
                                                  final String discountTypeUsed = (info['discount_type'] as String?) ?? 'fixed';
                                                  final double discountValueUsed = (info['discount_value'] as double?) ?? 0.0;
                                                  final double discountAmount = (info['discount_amount'] as double?) ?? 0.0;
                                                  final double effectiveTotalAfterDiscount = (info['effective_total'] as double?) ?? currentItemsTotal;

                                                  String discountLabel = '';
                                                  if (discountAmount > 0) {
                                                    if (discountTypeUsed == 'percent') {
                                                      discountLabel = ' • خصم ${discountValueUsed.toStringAsFixed(2)}% (${discountAmount.toStringAsFixed(2)})';
                                                    } else {
                                                      discountLabel = ' • خصم بالجنيه (${discountAmount.toStringAsFixed(2)})';
                                                    }
                                                  }

                                                  final double originalTotalBefore = (currentItemsTotal + refundedVal - addedVal);

                                                  // المبلغ الذي كان مسجلاً على الفاتورة قبل أي إجراءات
                                                  final double originalPaidBefore = (salePaidFromAPI).clamp(0.0, double.infinity);

                                                  // المبلغ الإضافي الذي سجله سجل المرتجع/الاستبدال (قد يكون موجبًا أو سالبًا)
                                                  final double extraPaid = paidDeltaSum;

                                                  // إجمالي ما دفعه العميل فعليًا بعد العملية
                                                  final double totalPaidByCustomer = (originalPaidBefore + extraPaid).clamp(0.0, double.infinity);
                                                  ...
                                                  final double displayEffectivePaid = (totalPaidByCustomer - originalChangeGiven).clamp(0.0, double.infinity);
                                                  ...
                                                  if (items.isEmpty && _groupReturnItemsByReturnId(saleId).isEmpty) {
                                                  ...
                                                  return Column(
                                                    ...
                                                    children: [
                                                      ...
                                                      const SizedBox(height: 12),
                                                      if (returnsById.isNotEmpty) Divider(color: AppColorsDark.mainColor),
                                                      ...returnsById.entries.map((entry) {
                                                        final rid = entry.key;
                                                        final rows = entry.value;
                                                        final meta = (saleReturnsCache[saleId] ?? []).firstWhere((rm) => (rm['id'] as int?) == rid, orElse: () => {});
